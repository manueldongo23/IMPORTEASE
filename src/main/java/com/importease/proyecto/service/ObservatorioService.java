...
import com.importease.proyecto.repository.ObservatorioDAO;
...
public class ObservatorioService {
    ...
    private final ObservatorioDAO dao = new ObservatorioDAO();
    ...
    public Map<String, Object> analizar(String codigo) {
        ...
        if (!dbCache.isEmpty()) {
            return construirAnalisis(hsCode, hs6, dbCache, "CACHE", DataConfidenceService.confidenceFor("CACHE"),
        ...
        fuenteEventoService.registrarCache(SOURCE, "OBSERVATORIO_HS", hs6);
        return construirAnalisis(hsCode, hs6, seed, apiKey.isBlank() ? "PENDIENTE_CREDENCIALES" : "CACHE",
                DataConfidenceService.confidenceFor(apiKey.isBlank() ? "PENDIENTE_CREDENCIALES" : "CACHE"),
                apiKey.isBlank()
    ...
    private Map<String, Object> consultarComtrade(String hsCode, String hs6, int anio, String apiKey) {
        ...
        try {
            ...
            if (response.statusCode() >= 200 && response.statusCode() < 300) {
                ...
                if (!stats.isEmpty()) {
                    double confidence = DataConfidenceService.confidenceFor("OFICIAL_API");
                    ...
                    fuenteEventoService.registrarOk(SOURCE, "OBSERVATORIO_HS", hs6, endpoint, "GET", response.statusCode(), response.body(), duration);
                ...
                fuenteEventoService.registrarError(SOURCE, "OBSERVATORIO_HS", hs6, endpoint, "GET", response.statusCode(), "HTTP " + response.statusCode(), duration);
            }
        } catch (IOException | InterruptedException e) {
            if (e instanceof InterruptedException) Thread.currentThread().interrupt();
            dao.registrarSync(hsCode, endpoint, null, "ERROR", e.getMessage());
            fuenteEventoService.registrarError(SOURCE, "OBSERVATORIO_HS", hs6, endpoint, "GET", 0, e.getMessage(), System.currentTimeMillis() - started);
