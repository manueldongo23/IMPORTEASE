...
public class ContribuyenteService {
    private final SunatRucService sunatApi = new SunatRucService();
    ...
    public Usuario validarRuc(String ruc) {
        ...
        // --- Estrategia 1: API externa ---
        try {
            Usuario real = sunatApi.consultarRuc(ruc);
            if (real != null && real.getRazonSocial() != null && !real.getRazonSocial().isBlank()) {
                new FuenteEventoService().registrarOk("RUC_TERCERO", "RUC_VALIDACION", ruc, null, "GET", 200, real.getRazonSocial(), 0);
        ...
        // --- Estrategia 2: Caché en base de datos (tabla validaciones_ruc) ---
        try {
            Usuario cached = buscarEnCache(ruc);
            if (cached != null) {
                LoggerUtil.info("RUC " + ruc + " encontrado en caché local de validaciones.");
                new FuenteEventoService().registrarOk("RUC_CACHE_LOCAL", "RUC_VALIDACION", ruc, null, "CACHE", 200, cached.getRazonSocial(), 0);
                return cached;
            }
        } catch (Exception e) {
        ...
        if (local != null) {
            ...
            new FuenteEventoService().registrarFallback(
