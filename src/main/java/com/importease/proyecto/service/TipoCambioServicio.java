...
/**
 * Servicio encargado de gestionar y obtener el tipo de cambio oficial del BCRP (TipoCambioServicio en español).
 */
public class TipoCambioServicio {
    ...
    // Campos de estado locales a cada hilo de ejecución para seguridad multihilo
    ...
    public BigDecimal obtenerTipoCambioSimulado() {
        new FuenteEventoServicio().registrarSimulado("FALLBACK_INTERNO", "TIPO_CAMBIO", "3.75", "Valor fijo de contingencia academica");
    ...
    public BigDecimal[] obtenerTipoCambioBCRP() {
        ...
        if (!cb.allowRequest()) {
            ...
            new FuenteEventoServicio().registrarFallback("BCRP_API", "TIPO_CAMBIO", BCRP_SERIES, "CircuitBreaker [BCRP_API_CB] activo");
        ...
        try {
            ...
            if (responseCode == 200) {
                ...
                lastPayloadHash.set(FuenteEventoServicio.hash(payload));
                ...
                cb.recordSuccess();
                new FuenteEventoServicio().registrarOk("BCRP_API", "TIPO_CAMBIO", BCRP_SERIES, BCRP_URL, "GET", responseCode, payload, System.currentTimeMillis() - start);
            } else {
                cb.recordFailure();
                new FuenteEventoServicio().registrarError("BCRP_API", "TIPO_CAMBIO", BCRP_SERIES, BCRP_URL, "GET", responseCode, "BCRP retorno codigo HTTP " + responseCode, System.currentTimeMillis() - start);
            }
        } catch (Exception e) {
            cb.recordFailure();
            new FuenteEventoServicio().registrarFallback("BCRP_API", "TIPO_CAMBIO", BCRP_SERIES, e.getMessage());
        }
        return new BigDecimal[]{compra, venta};
    }
        ...
        try (Connection con = ConexionDB.obtenerConexion();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs != null && rs.next()) {
                    lastFallback.set(false);
                    lastFuenteDato.set("CACHE");
                    new FuenteEventoServicio().registrarCache("BCRP_API", "TIPO_CAMBIO", hoy.toString());
    ...
    public Map<String, Object> obtenerTipoCambioDetalle() {
        ...
        data.put("confidence", DataConfidenceServicio.confidenceFor(lastFuenteDato.get()));
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs != null && rs.next()) {
                    ...
                    if (hasColumn(rs, "es_fallback")) {
                        ...
                        data.put("confidence", DataConfidenceServicio.confidenceFor(fallback ? "FALLBACK" : "OFICIAL_API"));
                    }
    ...
    public Map<String, Object> obtenerTipoCambioDetalleRapido() {
        ...
        try (Connection con = ConexionDB.obtenerConexion();
            ...
            if (rs != null && rs.next()) {
                ...
                data.put("confidence", DataConfidenceServicio.confidenceFor("CACHE"));
        ...
        data.put("confidence", DataConfidenceServicio.confidenceFor("FALLBACK"));
    ...
    private void guardarTipoCambio(Connection con, LocalDate fecha, BigDecimal compra, BigDecimal venta) throws Exception {
        ...
        try (PreparedStatement ps = con.prepareStatement(sqlV31)) {
            ...
            ps.setBigDecimal(11, BigDecimal.valueOf(DataConfidenceServicio.confidenceFor(lastFuenteDato.get())));
            ps.executeUpdate();
        } catch (Exception e) {
