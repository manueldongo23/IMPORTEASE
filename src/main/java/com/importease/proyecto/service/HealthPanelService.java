...
public class HealthPanelService {

    private static final int TOTAL_STEPS = 8;

    private final ImportacionDAO importacionDAO;
    private final GuidedFlowService guidedFlowService;
    private final NextActionService nextActionService;
    private final PlazoCriticoService plazoCriticoService;
    private final SunatDamScraperService sunatDamScraperService;

    public HealthPanelService() {
        this.importacionDAO = new ImportacionDAO();
        this.guidedFlowService = new GuidedFlowService();
        this.nextActionService = new NextActionService();
        this.plazoCriticoService = new PlazoCriticoService();
        this.sunatDamScraperService = new SunatDamScraperService();
    }

    public HealthPanelDTO calcularSalud(int expedienteId) {
        Importacion imp = null;
        try {
            imp = importacionDAO.buscarPorId(expedienteId);
        } catch (Exception e) {
            LoggerUtil.error("Error al buscar importacion en HealthPanelService", e);
        }

        HealthPanelDTO panel = new HealthPanelDTO();
        NextActionDTO nextAction = nextActionService.calcularSiguienteAccion(expedienteId);

        int completedSteps = 0;
        for (int i = 1; i <= TOTAL_STEPS; i++) {
            if (GuidedFlowService.isPasoCompleto(expedienteId, i)) {
                completedSteps++;
            }
        }
    ...
    private void consultarEstadoDamSunat(int expedienteId, Importacion imp, HealthPanelDTO panel) {
        if (imp == null) {
    ...
    private String calcularRiesgoPlazo(Importacion imp) {
        ...
        try (Connection con = ConexionDB.obtenerConexion()) {
            ...
            List<Map<String, Object>> plazos = PlazoCriticoService.calcularPlazos(con, imp, regimenCode, modalidadCode);
    ...
    private String calcularEstadoPreDam(Importacion imp) {
        ...
        try (Connection con = ConexionDB.obtenerConexion()) {
            ...
            return "HABILITADO";
        } catch (PredamValidationException e) {
            return "BLOQUEADO";
        } catch (Exception e) {
            LoggerUtil.warn("Error al validar PRE-DAM en HealthPanelService: " + e.getMessage());
            return "BLOQUEADO";
        }
    }
