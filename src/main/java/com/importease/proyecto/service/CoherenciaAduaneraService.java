...
public class CoherenciaAduaneraService {

    private final ImportacionDAO importacionDAO;

    public CoherenciaAduaneraService() {
        this.importacionDAO = new ImportacionDAO();
    }

    public List<CoherenciaIssueDTO> verificarCoherencia(int expedienteId) {
        List<CoherenciaIssueDTO> issues = new ArrayList<>();
        Importacion imp = null;

        try (Connection con = ConexionDB.obtenerConexion()) {
            imp = importacionDAO.buscarPorId(con, expedienteId);
            if (imp == null) {
                issues.add(new CoherenciaIssueDTO("ERROR", "Expediente no encontrado", "id", "ALTA", "Verifica que el ID del expediente sea correcto"));
                return issues;
            }
            ...
            issues.add(new CoherenciaIssueDTO("ERROR", "Error interno al verificar coherencia: " + e.getMessage(), null, "ALTA", "Reintente la operaciÃƒÂ³n"));
        }

        if (issues.isEmpty()) {
            issues.add(new CoherenciaIssueDTO("INFO", "Todos los controles de coherencia aduanera pasaron correctamente", null, "BAJA", "El expediente estÃƒÂ¡ listo para continuar"));
        }

        return issues;
    }

    private void verificarManifiestoBL(Connection con, int expedienteId, List<CoherenciaIssueDTO> issues) {
        ...
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (!rs.next()) {
                    issues.add(new CoherenciaIssueDTO("ERROR", "No existe manifiesto de carga registrado", "manifiestoId", "ALTA",
                    ...
                    if (numDoc == null || numDoc.isBlank()) {
                        issues.add(new CoherenciaIssueDTO("WARNING", "El manifiesto no tiene un BL/AWB asociado", "numeroDocumento", "MEDIA",
    ...
    private void verificarReimportacion(Importacion imp, List<CoherenciaIssueDTO> issues) {
        ...
        if (esReimportacion) {
            issues.add(new CoherenciaIssueDTO("WARNING", "RÃƒÂ©gimen de reimportaciÃƒÂ³n: se requiere DAM de exportaciÃƒÂ³n precedente",
                "regimen", "ALTA", "Verifica que exista una declaraciÃƒÂ³n de exportaciÃƒÂ³n previa regularizada dentro de 12 meses"));
        }
    }

    private void verificarAdmisionTemporal(Connection con, int expedienteId, List<CoherenciaIssueDTO> issues) {
        try {
            ...
            try (PreparedStatement ps = con.prepareStatement(sql)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (!rs.next()) {
                        issues.add(new CoherenciaIssueDTO("INFO", "No se detectÃƒÂ³ garantÃƒÂ­a. Si el rÃƒÂ©gimen es AdmisiÃƒÂ³n Temporal, se requiere garantÃƒÂ­a",
    ...
    private void verificarMercanciaRestringida(Connection con, int expedienteId, Importacion imp, List<CoherenciaIssueDTO> issues) {
            ...
            try (PreparedStatement ps = con.prepareStatement(sql)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next() && rs.getInt(1) == 0) {
                        issues.add(new CoherenciaIssueDTO("ERROR",
    ...
    private void verificarFob(Importacion imp, List<CoherenciaIssueDTO> issues) {
        if (imp.getValorFob() <= 0) {
            issues.add(new CoherenciaIssueDTO("ERROR", "El valor FOB debe ser mayor a 0 USD",
                "fob", "ALTA", "Ingresa un valor FOB positivo"));
        } else if (imp.getValorFob() > 10_000_000) {
            issues.add(new CoherenciaIssueDTO("WARNING", "El valor FOB excede los 10,000,000 USD, puede requerir validaciÃƒÂ³n adicional",
                "fob", "MEDIA", "Verifica que el valor FOB sea correcto"));
        }
    }

    private void verificarDespachoFechas(Connection con, Importacion imp, List<CoherenciaIssueDTO> issues) {
        if (imp.getEstado() == null) return;
                ...
                if (rs.next()) {
                    ...
                    if (fechaLlegada != null) {
                        ...
                        if (esAnticipado && !llegada.isAfter(now)) {
                            issues.add(new CoherenciaIssueDTO("WARNING",
    ...
    private void verificarHsCode(Importacion imp, List<CoherenciaIssueDTO> issues) {
        if (imp.getHsCode() == null || imp.getHsCode().isBlank()) {
            issues.add(new CoherenciaIssueDTO("ERROR", "HS Code no registrado",
                "hsCode", "ALTA", "Clasifica tu mercancÃƒÂ­a con un HS Code de 6 a 10 dÃƒÂ­gitos"));
        } else if (!imp.getHsCode().matches("\\d{6,10}")) {
            issues.add(new CoherenciaIssueDTO("ERROR", "HS Code debe tener entre 6 y 10 dÃƒÂ­gitos numÃƒÂ©ricos",
