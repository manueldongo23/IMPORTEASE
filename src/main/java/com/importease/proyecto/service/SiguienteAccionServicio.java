...
public class SiguienteAccionServicio {

    private final ImportacionRepositorio importacionRepositorio;

    public SiguienteAccionServicio() {
        this.importacionRepositorio = new ImportacionRepositorio();
    }

    SiguienteAccionServicio(ImportacionRepositorio importacionRepositorio) {
        this.importacionRepositorio = importacionRepositorio;
    }

    public SiguienteAccionDTO calcularSiguienteAccion(int expedienteId) {
        Importacion imp = null;
        try {
            imp = importacionRepositorio.buscarPorId(expedienteId);
        } catch (Exception e) {
            LoggerUtil.error("Error al buscar importacion en SiguienteAccionServicio", e);
        }
    ...
    private static boolean isPasoCompleto(int expedienteId, int paso) {
        String estado = FlujoGuiadoServicio.getPasoEstado(expedienteId, paso);
