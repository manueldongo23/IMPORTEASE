...
public class AduanasDamServicio {
    ...
    private CalculadoraTributaria.ResultadoTributario recalcularTributos(Importacion imp) {
        try {
            HsCodeRepositorio hsDao = new HsCodeRepositorio();
            HsCode hs = hsDao.obtenerPorCodigo(imp.getHsCode());
            if (hs == null) {
                ArancelServicio arancelServicio = new ArancelServicio();
                hs = arancelServicio.consultarArancel(imp.getHsCode());
            }
            Usuario usuario = new UsuarioRepositorio().buscarPorId(imp.getUsuarioId());
            if (hs != null) {
