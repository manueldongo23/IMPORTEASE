...
import com.importease.proyecto.service.PredamValidationException;
import com.importease.proyecto.service.PredamValidationService;
...
public class GuidedFlowRuleService {
    private final ImportacionDAO importacionDAO;

    public GuidedFlowRuleService() {
        this(new ImportacionDAO());
    }

    public GuidedFlowRuleService(ImportacionDAO importacionDAO) {
    ...
    public String validateStep(Connection con, int paso, int expedienteId) {
        try {
            switch (paso) {
                case GuidedFlowService.PASO_INTENCION:
                    return null;
                case GuidedFlowService.PASO_DATOS_BASICOS:
                    return validarDatosBasicos(con, expedienteId);
                case GuidedFlowService.PASO_CLASIFICACION:
                    return validarClasificacion(con, expedienteId);
                case GuidedFlowService.PASO_TRANSPORTE:
                    return validarTransporte(con, expedienteId);
                case GuidedFlowService.PASO_DOCUMENTOS:
                    return validarDocumentos(con, expedienteId);
                case GuidedFlowService.PASO_COHERENCIA:
                    return validarCoherencia(expedienteId);
                case GuidedFlowService.PASO_DTA_PRE_DAM:
                    return validarPredam(con, expedienteId);
                case GuidedFlowService.PASO_REVISION_FINAL:
                    return null;
                default:
    ...
    private String validarCoherencia(int expedienteId) {
        CoherenciaAduaneraService coherenciaService = new CoherenciaAduaneraService();
        List<CoherenciaIssueDTO> issues = coherenciaService.verificarCoherencia(expedienteId);
        StringBuilder errorMsg = new StringBuilder();
        for (CoherenciaIssueDTO issue : issues) {
