...
public class PermisoRespuestaRepository extends PermisoRepositorySupport {

    public boolean guardarDatosSolicitud(int solicitudId, List<SolicitudPermisoDato> datos) {
        SolicitudPermisoDatoJpaRepository repo = getDatoRepo();
        if (repo != null) {
            try {
    ...
    public boolean guardarRespuestasPermiso(int operacionId, Map<Integer, String> respuestas) {
        RespuestaPermisoOperacionJpaRepository repo = getRespuestaRepo();
        if (repo != null) {
            try {
