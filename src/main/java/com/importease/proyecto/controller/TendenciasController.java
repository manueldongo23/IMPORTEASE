...
import com.importease.proyecto.repository.BusquedaDAO;
import com.importease.proyecto.service.DataConfidenceService;
import com.importease.proyecto.service.EventoUsuarioService;
import com.importease.proyecto.service.FuenteEventoService;
...
public class TendenciasController extends HttpServlet {
    private final Gson gson = new Gson();
    private final BusquedaDAO busquedaDAO = new BusquedaDAO();
    private final EventoUsuarioService eventoUsuarioService = new EventoUsuarioService();
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (!"/registrar".equals(path)) {
            resp.setStatus(404);
            write(resp, ResponseEnvelope.error("ENDPOINT_NOT_FOUND", "Endpoint de tendencias no encontrado", "BD_LOCAL", false));
            return;
        }
        ...
        try {
            ...
            if (termino == null || termino.isBlank()) {
                resp.setStatus(400);
                write(resp, ResponseEnvelope.error("EMPTY_TERM", "No se puede registrar una busqueda vacia", "BD_LOCAL", false));
                return;
            }

            Integer usuarioId = getUsuarioId(req);
            String tipoSeguro = (tipo == null || tipo.isBlank()) ? "PRODUCTO" : tipo.toUpperCase();
            busquedaDAO.registrar(usuarioId, termino, hsCode, tipoSeguro);
            ...
            eventoUsuarioService.registrar(
                    usuarioId,
            ...
            write(resp, ResponseEnvelope.ok(data, "BD_LOCAL", "BD_LOCAL", DataConfidenceService.confidenceFor("BD_LOCAL")));
        } catch (Exception e) {
            LoggerUtil.error("Error registrando tendencia", e);
            fuenteEventoService.registrarError("BD_LOCAL", "TENDENCIA_REGISTRAR", null, null, "POST", 500, e.getMessage(), 0);
            resp.setStatus(500);
            write(resp, ResponseEnvelope.error("TRENDS_REGISTER_ERROR", "No se pudo registrar la tendencia", "BD_LOCAL", false));
        }
    }
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if ("/top-productos".equals(path) || "/top-busquedas".equals(path)) {
                ...
                data = busquedaDAO.topProductosBuscados(dias, limit);
                ...
                data = busquedaDAO.topHs(dias, limit);
            } else if ("/top-capitulos".equals(path)) {
                data = busquedaDAO.topCategorias(dias, limit);
                ...
                data = busquedaDAO.conversionBasica(dias);
            } else if ("/funnel".equals(path)) {
                data = busquedaDAO.eventosPorTipo(dias, limit);
            } else if ("/abandono".equals(path)) {
                data = busquedaDAO.eventosPorTipo(dias, limit);
            } else if ("/ultimas".equals(path)) {
                Integer uid = getUsuarioId(req);
                if (uid != null) {
                    java.util.List<java.util.Map<String, Object>> all = busquedaDAO.ultimasBusquedas(limit);
            ...
            resp.setStatus(500);
            write(resp, ResponseEnvelope.error("TRENDS_QUERY_ERROR", "No se pudieron consultar las tendencias", "BD_LOCAL", false));
        }
    }
