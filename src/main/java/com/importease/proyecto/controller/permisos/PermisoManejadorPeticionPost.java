...
public class PermisoManejadorPeticionPost {
    private final PermisoControladorSoporte support;

    public PermisoManejadorPeticionPost(PermisoControladorSoporte support) {
    ...
    public void handle(HttpServletRequest req, HttpServletResponse resp, int usuarioId) throws IOException {
        ...
        if ("/evaluar".equals(path)) {
            new PermisoManejadorPeticionGet(support).handle(req, resp, usuarioId);
    ...
    private void handleCambiarEstado(HttpServletRequest req, HttpServletResponse resp, int usuarioId) throws IOException {
        ...
        if (sol == null || sol.getUsuarioId() != usuarioId) {
            resp.setStatus(403);
