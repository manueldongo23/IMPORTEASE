...
public class AduanasController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (usuarioId == null) {
            ...
            write(resp, ResponseEnvelope.error("NO_SESSION", "Sesion requerida", AduanasService.SOURCE, false));
            return;
        }
        ...
        try {
            ...
            if ("/expediente".equals(path)) {
                data = service.obtenerExpediente(usuarioId, getOperacionId(req));
            } else if ("/timeline".equals(path)) {
                data = service.obtenerTimeline(usuarioId, getOperacionId(req));
            } else if ("/alertas".equals(path)) {
                data = service.obtenerAlertas(usuarioId, getOperacionId(req));
            } else if ("/base-legal".equals(path)) {
                data = service.obtenerBaseLegal(usuarioId, getOperacionId(req));
            } else {
                resp.setStatus(HttpServletResponse.SC_NOT_FOUND);
                write(resp, ResponseEnvelope.error("ENDPOINT_NOT_FOUND", "Endpoint aduanero no encontrado", AduanasService.SOURCE, false));
                return;
            }
            EnvelopeMetadata meta = resolveEnvelopeMetadata(data);
            write(resp, ResponseEnvelope.ok(data, meta.source, meta.sourceType, meta.confidence));
        } catch (SecurityException e) {
            resp.setStatus(HttpServletResponse.SC_FORBIDDEN);
            write(resp, ResponseEnvelope.error("FORBIDDEN", "No autorizado para esta operacion", AduanasService.SOURCE, false));
        } catch (IllegalArgumentException e) {
            resp.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            write(resp, ResponseEnvelope.error("BAD_REQUEST", e.getMessage(), AduanasService.SOURCE, false));
        } catch (Exception e) {
            resp.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            write(resp, ResponseEnvelope.error("ADUANAS_ERROR", "No se pudo procesar la solicitud aduanera", AduanasService.SOURCE, false));
        }
    }
    ...
    private EnvelopeMetadata resolveEnvelopeMetadata(Object data) {
        String source = AduanasService.SOURCE;
        String sourceType = AduanasService.SOURCE_TYPE;
        ...
        if (candidate != null) {
            ...
            confidence = confidenceValue instanceof Number n
                    ? n.doubleValue()
                    : DataConfidenceService.confidenceFor(sourceType);
        }
