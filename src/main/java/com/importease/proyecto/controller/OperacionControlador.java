...
public class OperacionControlador extends HttpServlet {
    private final OperacionRepositorio opDao = new OperacionRepositorio();
    ...
    private void numerarOperacion(HttpServletRequest req, HttpServletResponse resp, PrintWriter out) throws IOException {
        ...
        Usuario u = new UsuarioRepositorio().buscarPorId(op.getUsuarioId());
        HsCode hs = new ArancelServicio().consultarArancelLocal(op.getHsCode());
        RiskScoringServicio.ResultadoRiesgo risk = RiskScoringServicio.evaluarRiesgo(
