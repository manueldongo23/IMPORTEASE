...
import com.importease.proyecto.service.TrackingService;
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if ("/listar".equals(path)) {
                data = trackingService.listar(usuarioId);
            ...
            if ("/detalle".equals(path)) {
                ...
                data = trackingService.detalle(id, usuarioId);
                if (data == null) {
                    resp.setStatus(404);
                    write(resp, ResponseEnvelope.error("TRACKING_NOT_FOUND", "Tracking no encontrado", "BD_LOCAL", false));
                    return;
                }
                write(resp, ResponseEnvelope.ok(data, "BD_LOCAL", "BD_LOCAL", DataConfidenceService.confidenceFor("BD_LOCAL")));
                return;
            ...
            if ("/eventos".equals(path)) {
                long id = parseLong(req.getParameter("id"), -1);
                data = trackingService.eventos(id, usuarioId);
                write(resp, ResponseEnvelope.ok(data, "BD_LOCAL", "BD_LOCAL", DataConfidenceService.confidenceFor("BD_LOCAL")));
                return;
            }
            resp.setStatus(404);
            write(resp, ResponseEnvelope.error("ENDPOINT_NOT_FOUND", "Endpoint de tracking no encontrado", "BD_LOCAL", false));
        } catch (Exception e) {
            resp.setStatus(500);
            write(resp, ResponseEnvelope.error("TRACKING_QUERY_ERROR", "No se pudo consultar tracking", "BD_LOCAL", false));
        }
    }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            if ("/registrar".equals(path)) {
                ...
                Map<String, Object> data = trackingService.registrar(usuarioId, sessionId, body, req.getRemoteAddr(), req.getHeader("User-Agent"));
                String sourceType = str(data.get("sourceType"), "MANUAL_VERIFICADO");
                write(resp, ResponseEnvelope.ok(data, "USUARIO", sourceType, DataConfidenceService.confidenceFor(sourceType)));
                return;
            }
            if ("/sincronizar".equals(path)) {
                ...
                Map<String, Object> data = trackingService.sincronizar(id, usuarioId, sessionId, req.getRemoteAddr(), req.getHeader("User-Agent"));
                String sourceType = str(data.get("sourceType"), "PENDIENTE_CREDENCIALES");
                write(resp, ResponseEnvelope.ok(data, "TRACKING_API", sourceType, DataConfidenceService.confidenceFor(sourceType)));
                return;
            }
            resp.setStatus(404);
            write(resp, ResponseEnvelope.error("ENDPOINT_NOT_FOUND", "Endpoint de tracking no encontrado", "BD_LOCAL", false));
            ...
            resp.setStatus(400);
            write(resp, ResponseEnvelope.error("TRACKING_BAD_REQUEST", e.getMessage(), "BD_LOCAL", false));
        } catch (Exception e) {
            resp.setStatus(500);
            write(resp, ResponseEnvelope.error("TRACKING_MUTATION_ERROR", "No se pudo procesar tracking", "BD_LOCAL", false));
        }
    }
