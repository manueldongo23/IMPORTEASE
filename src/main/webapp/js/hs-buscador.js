// hs-buscador.js – Copia directa de js/buscador.js (manteniendo nombres originales).
// Este archivo es estático; las variables dinámicas se leen del objeto global ImportEase.
// Lectura de variables de contexto (CSP nonce ya gestionado en el JSP).
...
// ------------------------------------------------------------
// Contenido original de js/buscador.js (746 líneas)
// ------------------------------------------------------------
...
function initBuscadorEvents() {
    ...
    // Vinculación de búsquedas populares
    document.querySelectorAll('.hs-popular-tag').forEach(btn => {
        btn.addEventListener('click', () => {
            const term = btn.getAttribute('data-tag');
            if (term && input) {
                input.value = term;
                onInput(term);
                buscar();
    ...
    // Copiar código arancelario
    const btnCopiar = document.getElementById('btnCopiarCodigo');
    if (btnCopiar) {
        btnCopiar.addEventListener('click', copiarCodigo);
    }

    // Enlace de mentor (Ayuda Rápida)
    const btnMentor = document.getElementById('btnMentorLink');
    if (btnMentor) {
        btnMentor.addEventListener('click', () => {
            window.open('https://www.sunat.gob.pe/orientacionaduanera/tlc/', '_blank');
        });
    }

    // Botón VUCE link
    const btnVuce = document.getElementById('vuceBadgeLink');
    if (btnVuce) {
        btnVuce.addEventListener('click', () => {
            const url = btnVuce.getAttribute('data-url');
            if (url) window.open(url, '_blank');
        });
    }

    // Usar en importación (Wizard)
    const btnUsar = document.getElementById('btnUsarEnImportacion');
    if (btnUsar) {
        btnUsar.addEventListener('click', usarCodigoEnImportacion);
    }

    // Ver mercado
    const btnVerMercado = document.getElementById('btnVerMercado');
    if (btnVerMercado) {
        btnVerMercado.addEventListener('click', verMercadoHs);
    }
...
async function liveSearch(val) {
    ...
    try {
        const res = await fetch(ctx + '/api/hs/sugerencias?termino=' + encodeURIComponent(cleanVal), {
...
function renderAC(lista) {
    ...
    lista.forEach((hs, i) => {
        ...
        item.innerHTML = `
            ...
            <div class=\"text-right\">${restrictedBadge}</div>
        `;
...
function registrarBusqueda(termino, hsCode, tipo) {
    try {
        fetch(ctx + '/api/tendencias/registrar', {
            method: 'POST',
            headers: {
...
async function buscar() {
    ...
    try {
        ...
        const res = await fetch(ctx + '/api/hs/buscar?' + param);
        ...
        mostrarError('No pudimos consultar la fuente. Inténtalo nuevamente en unos segundos.');
    } finally {
        setBtnLoading(false);
    }
}
...
function mostrarResultado(hs) {
    const code = (hs.hsCode || '').replace(/\./g, '').padEnd(10, '0');
    actualizarDigito('digitCapitulo', code.substring(0, 2) || '--');
    actualizarDigito('digitPartida', code.substring(2, 4) || '--');
    actualizarDigito('digitSubpartida', code.substring(4, 6) || '--');
    actualizarDigito('digitItem', code.substring(6, 8) || '--');
    actualizarDigito('digitSubitem', code.substring(8, 10) || '--');
    codigoActual = code;
    selectedHsForWizard = {
    ...
    setTimeout(() => {
        desc.innerText = hs.descripcion || 'Descripción referencial no disponible.';
    ...
    if (badge) {
        ...
        const titleEl = document.getElementById('vuceBadgeTitle');
        const descEl = document.getElementById('vuceBadgeDesc');
        const linkEl = document.getElementById('vuceBadgeLink');
        
        if (hs.requiereVUCE) {
            badge.className = 'hs-alert-banner warning fade-up';
            if (titleEl) titleEl.innerText = 'REVISA PERMISO CON ' + (hs.entidadVUCE || 'SUNAT') + '.';
            if (descEl) descEl.innerText = 'Antes de comprar o embarcar, confirma si necesitas autorización.';
            if (linkEl) {
                linkEl.setAttribute('data-url', ENTIDAD_URLS[hs.entidadVUCE] || 'https://www.vuce.gob.pe');
                linkEl.querySelector('span').innerText = 'Ver entidad';
            }
        } else {
            badge.className = 'hs-alert-banner success fade-up';
            if (titleEl) titleEl.innerText = 'SIN PERMISO EVIDENTE.';
            if (descEl) descEl.innerText = 'Con la información disponible, no vemos una alerta de permiso.';
            if (linkEl) {
                linkEl.setAttribute('data-url', 'https://www.vuce.gob.pe');
                linkEl.querySelector('span').innerText = 'Ver fuente oficial';
            }
        }
    }
    ...
    setTimeout(function() {
        if (av === 0) {
            ...
            mentor.innerHTML = '<strong>Impuesto base detectado:</strong> aparece una tasa de ' + esc(av) + '%. Compara origen y documentos antes de pagar al proveedor.';
        }
...
function mostrarInfoProducto(code, desc, av, reqVuce, entVuce) {
    ...
    var html = '';
    html += '<tr><td class="hs-info-label">Categoría</td><td class="hs-info-value">' + esc(info.categoria) + '</td></tr>';
    html += '<tr><td class="hs-info-label">Grupo del producto</td><td class="hs-info-value">' + esc((cap < 10 ? '0' : '') + cap + ' - ' + info.capitulo) + '</td></tr>';
    html += '<tr><td class="hs-info-label">Orígenes frecuentes</td><td class="hs-info-value">' + esc(info.origenes) + '</td></tr>';
    html += '<tr><td class="hs-info-label">Impuestos aproximados</td><td class="hs-info-value"><span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-200/50">' + esc((av + 18).toFixed(1)) + '%</span></td></tr>';
    if (reqVuce) {
        html += '<tr><td class="hs-info-label">¿Quién podría revisar?</td><td class="hs-info-value text-amber-600 font-bold">' + esc(entVuce || 'Entidad por confirmar') + '</td></tr>';
    }
    content.innerHTML = html;
    if (info.tip) {
        tipDiv.classList.remove('hidden');
        tipDiv.innerHTML = esc(info.tip);
    } else {
...
function setBtnLoading(loading) {
    ...
    btn.innerHTML = loading ? 'Buscando...' : 'Buscar código';
    if (loading) {
...
const ADUANABOT_KEYWORD_MAP = [
    ...
    },
    {
        ...
        code: '3004902900',
        desc: 'Medicamentos acondicionados para la venta al menor',
        taxes: 'Ad-Valorem: 0%, IGV: 16%, IPM: 2%',
    ...
    setTimeout(async () => {
        ...
        try {
            ...
            if (SUGGESTIONS_CACHE.has(queryLower)) {
                ...
                searchAbortController = new AbortController();
                const res = await fetch(ctx + '/api/hs/sugerencias?termino=' + encodeURIComponent(query), {
                    signal: searchAbortController.signal
                });
                if (res.ok) {
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => { initBuscadorEvents(); initAduanaBot(); });
} else {
    initBuscadorEvents();
    initAduanaBot();
}
