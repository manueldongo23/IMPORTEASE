/**
 * JavaScript module for registration view (registro.jsp)
 * Reads context path configuration from window.ImportEase.ctx
 */
async function validarRuc(event) {
    ...
    const ctx = window.ImportEase?.ctx || '';
    
    try {
        const res = await fetch(ctx + '/api/usuario/validarRuc?ruc=' + ruc);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (btnToggle) {
        btnToggle.addEventListener('click', () => {
            togglePasswordVisibility('password', btnToggle);
        });
    }
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            const res = await fetch(ctx + '/api/usuario/registro', {
                method: 'POST',
