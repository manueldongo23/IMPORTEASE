...
public class ImportacionServiceTest {
    ...
    public void setUp() {
        ...
        // Mock external API to return a fixed exchange rate
        when(externalApi.obtenerTipoCambioSunat()).thenReturn(3.80);
    }
    ...
    public void testGenerarOperacionComercialTasaEstandar() throws SQLException {
        ...
        try (MockedStatic<ConexionDB> mockedConexion = Mockito.mockStatic(ConexionDB.class);
            ...
            // IGV (16%) = 907.36
            // IPM (2%) = 113.42
            // PercepciÃ³n estÃ¡ndar (3.5%) de (Base IGV + IGV + IPM) => (5671 + 907.36 + 113.42) = 6691.78 * 0.035 = 234.21
            Importacion imp = importacionService.generarOperacion(1, "8517130000",
    ...
    public void testGenerarOperacionPreDamPrefix() throws SQLException {
        ...
        try (MockedStatic<ConexionDB> mockedConexion = Mockito.mockStatic(ConexionDB.class);
            ...
            assertTrue(imp.getNumeroDam().startsWith("PRE-DAM-"), "El nÃºmero DAM debe iniciar con PRE-DAM- para ser considerado referencial/simulado.");
        }
    }
    ...
    public void testCambiarEstadoNacionalizadaSinDocumentos() throws SQLException {
        try (MockedStatic<ConexionDB> mockedConexion = Mockito.mockStatic(ConexionDB.class)) {
            ...
            when(importacionDAO.validarDocumentosParaDespacho(any(Connection.class), eq(123))).thenReturn(false);

            boolean success = importacionService.cambiarEstado(123, "NACIONALIZADA");

            assertFalse(success, "No se debe permitir la nacionalizaciÃ³n si faltan documentos validados.");
            verify(importacionDAO, never()).actualizarEstado(any(Connection.class), eq(123), anyString());
        }
    }
}
